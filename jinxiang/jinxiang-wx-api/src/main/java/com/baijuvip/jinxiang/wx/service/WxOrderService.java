...
import com.baijuvip.jinxiang.core.express.ExpressService;
import com.baijuvip.jinxiang.core.express.dao.ExpressInfo;
import com.baijuvip.jinxiang.core.notify.NotifyService;
import com.baijuvip.jinxiang.core.notify.NotifyType;
import com.baijuvip.jinxiang.core.qcode.QCodeService;
import com.baijuvip.jinxiang.core.system.SystemConfig;
import com.baijuvip.jinxiang.core.util.DateTimeUtil;
...
import com.baijuvip.jinxiang.db.util.OrderHandleOption;
import com.baijuvip.jinxiang.db.util.OrderUtil;
import com.baijuvip.jinxiang.core.util.IpUtil;
...
import static com.baijuvip.jinxiang.wx.util.WxResponseCode.*;
...
public class WxOrderService {
    ...
    @Autowired
    private JinxiangOrderGoodsService orderGoodsService;
    @Autowired
    private JinxiangAddressService addressService;
    @Autowired
    private JinxiangCartService cartService;
    @Autowired
    ...
    public Object list(Integer userId, Integer showType, Integer page, Integer limit, String sort, String order) {
        ...
        List<JinxiangOrder> orderList = orderService.queryByOrderStatus(userId, orderStatus, page, limit, sort, order);

        List<Map<String, Object>> orderVoList = new ArrayList<>(orderList.size());
        for (JinxiangOrder o : orderList) {
            ...
            JinxiangGroupon groupon = grouponService.queryByOrderId(o.getId());
            if (groupon != null) {
            ...
            List<JinxiangOrderGoods> orderGoodsList = orderGoodsService.queryByOid(o.getId());
            List<Map<String, Object>> orderGoodsVoList = new ArrayList<>(orderGoodsList.size());
            for (JinxiangOrderGoods orderGoods : orderGoodsList) {
    ...
    public Object detail(Integer userId, Integer orderId) {
        ...
        // 订单信息
        JinxiangOrder order = orderService.findById(orderId);
        if (null == order) {
        ...
        orderVo.put("integral", order.getIntegral());
        orderVo.put("orderStatusText", OrderUtil.orderStatusText(order));
        orderVo.put("handleOption", OrderUtil.build(order));
        orderVo.put("expCode", order.getShipChannel());
        orderVo.put("expNo", order.getShipSn());

        List<JinxiangOrderGoods> orderGoodsList = orderGoodsService.queryByOid(order.getId());
    ...
    public Object submit(Integer userId, String body) {
        ...
        if (user == null) {
            return ResponseUtil.fail(600, "用户不存在");
        }

        //如果是团购项目,验证活动是否有效
        if (grouponRulesId != null && grouponRulesId > 0) {
            JinxiangGrouponRules rules = grouponRulesService.queryById(grouponRulesId);
            //找不到记录
            if (rules == null) {
                return ResponseUtil.badArgument();
            }
        ...
        // 收货地址
        JinxiangAddress checkedAddress = addressService.query(userId, addressId);
        ...
        JinxiangGrouponRules grouponRules = grouponRulesService.queryById(grouponRulesId);
        if (grouponRules != null) {
            grouponPrice = grouponRules.getDiscount();
        }

        // 货品价格
        List<JinxiangCart> checkedGoodsList = null;
        if (cartId.equals(0)) {
            checkedGoodsList = cartService.queryByUidAndChecked(userId);
        } else {
            JinxiangCart cart = cartService.findById(cartId);
        ...
        for (JinxiangCart checkGoods : checkedGoodsList) {
        ...
        if (couponId != 0 && couponId != -1) {
            JinxiangCoupon coupon = couponVerifyService.checkCoupon(userId, couponId, checkedGoodsPrice);
            if (coupon == null) {
                return ResponseUtil.badArgumentValue();
            }
        ...
        userIntegral.setRemark("用户积分兑换订单" + order.getOrderSn() + "消费积分：" + integral);
        ...
        for (JinxiangCart cartGoods : checkedGoodsList) {
            // 订单商品
            JinxiangOrderGoods orderGoods = new JinxiangOrderGoods();
        ...
        for (JinxiangCart checkGoods : checkedGoodsList) {
            ...
            JinxiangGoodsProduct product = productService.findById(productId);
        ...
        //如果是团购项目，添加团购信息
        if (grouponRulesId != null && grouponRulesId > 0) {
            JinxiangGroupon groupon = new JinxiangGroupon();
            ...
            if (grouponLinkId != null && grouponLinkId > 0) {
                //参与的团购记录
                JinxiangGroupon baseGroupon = grouponService.queryById(grouponLinkId);
    ...
    public Object cancel(Integer userId, String body) {
        ...
        // 商品货品数量增加
        List<JinxiangOrderGoods> orderGoodsList = orderGoodsService.queryByOid(orderId);
        for (JinxiangOrderGoods orderGoods : orderGoodsList) {
    ...
    public Object payNotify(HttpServletRequest request, HttpServletResponse response) {
        ...
        JinxiangOrder order = orderService.findBySn(orderSn);
        if (order == null) {
        ...
        //  支付成功，有团购信息，更新团购信息
        JinxiangGroupon groupon = grouponService.queryByOrderId(order.getId());
        if (groupon != null) {
            JinxiangGrouponRules grouponRules = grouponRulesService.queryById(groupon.getRulesId());
    ...
    public Object goods(Integer userId, Integer orderId, Integer goodsId) {
        ...
        List<JinxiangOrderGoods> orderGoodsList = orderGoodsService.findByOidAndGid(orderId, goodsId);
        ...
        JinxiangOrderGoods orderGoods = orderGoodsList.get(0);
    ...
    public Object comment(Integer userId, String body) {
        ...
        JinxiangOrderGoods orderGoods = orderGoodsService.findById(orderGoodsId);
        if (orderGoods == null) {
            return ResponseUtil.badArgumentValue();
        }
        ...
        // 1. 创建评价
        JinxiangComment comment = new JinxiangComment();
