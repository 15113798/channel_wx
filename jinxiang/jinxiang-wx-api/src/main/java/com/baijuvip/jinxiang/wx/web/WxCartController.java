...
import static com.baijuvip.jinxiang.wx.util.WxResponseCode.GOODS_NO_STOCK;
import static com.baijuvip.jinxiang.wx.util.WxResponseCode.GOODS_UNSHELVE;
...
public class WxCartController {
    ...
    public Object index(@LoginUser Integer userId) {
        ...
        List<JinxiangCart> cartList = cartService.queryByUid(userId);
        Integer goodsCount = 0;
        BigDecimal goodsAmount = new BigDecimal(0.00);
        Integer checkedGoodsCount = 0;
        BigDecimal checkedGoodsAmount = new BigDecimal(0.00);
        for (JinxiangCart cart : cartList) {
     ...
     */
    @PostMapping("add")
    public Object add(@LoginUser Integer userId, @RequestBody JinxiangCart cart) {
        if (userId == null) {
            return ResponseUtil.unlogin();
        }
        if (cart == null) {
        ...
        //判断购物车中是否存在此规格商品
        JinxiangCart existCart = cartService.queryExist(goodsId, productId, userId);
        if (existCart == null) {
            //取得规格的信息,判断规格库存
     ...
     */
    @PostMapping("fastadd")
    public Object fastadd(@LoginUser Integer userId, @RequestBody JinxiangCart cart) {
        if (userId == null) {
            return ResponseUtil.unlogin();
        }
        if (cart == null) {
     ...
     */
    @PostMapping("update")
    public Object update(@LoginUser Integer userId, @RequestBody JinxiangCart cart) {
        if (userId == null) {
            return ResponseUtil.unlogin();
        }
        if (cart == null) {
        ...
        //判断是否存在该订单
        // 如果不存在，直接返回错误
        JinxiangCart existCart = cartService.findById(id);
        if (existCart == null) {
    ...
    public Object checkout(@LoginUser Integer userId, Integer cartId, Integer addressId, Integer couponId, Integer grouponRulesId) {
        ...
        // 收货地址
        JinxiangAddress checkedAddress = null;
        if (addressId == null || addressId.equals(0)) {
            checkedAddress = addressService.findDefault(userId);
            // 如果仍然没有地址，则是没有收获地址
            // 返回一个空的地址id=0，这样前端则会提醒添加地址
            if (checkedAddress == null) {
                checkedAddress = new JinxiangAddress();
        ...
        for (JinxiangCart cart : checkedGoodsList) {
        ...
        List<JinxiangCouponUser> couponUserList = couponUserService.queryAll(userId);
        for(JinxiangCouponUser couponUser : couponUserList){
            JinxiangCoupon coupon = couponVerifyService.checkCoupon(userId, couponUser.getCouponId(), checkedGoodsPrice);
            if(coupon == null){
                continue;
            }
