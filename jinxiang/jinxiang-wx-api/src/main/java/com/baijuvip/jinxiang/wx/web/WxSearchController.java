...
public class WxSearchController {
    ...
    @Autowired
    private JinxiangKeywordService keywordsService;
    @Autowired
    ...
    public Object index(@LoginUser Integer userId) {
        //取出输入框默认的关键词
        JinxiangKeyword defaultKeyword = keywordsService.queryDefault();
        //取出热闹关键词
        List<JinxiangKeyword> hotKeywordList = keywordsService.queryHots();

        List<JinxiangSearchHistory> historyList = null;
        if (userId != null) {
            //取出用户历史关键字
    ...
    public Object helper(@NotEmpty String keyword,
        ...
        List<JinxiangKeyword> keywordsList = keywordsService.queryByKeyword(keyword, page, limit);
        String[] keys = new String[keywordsList.size()];
        int index = 0;
        for (JinxiangKeyword key : keywordsList) {
