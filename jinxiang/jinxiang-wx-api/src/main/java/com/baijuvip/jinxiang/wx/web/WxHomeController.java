...
public class WxHomeController {
    ...
    private List<Map> getCategoryList() {
        ...
        List<JinxiangCategory> catL1List = categoryService.queryL1WithoutRecommend(0, SystemConfig.getCatlogListLimit());
        for (JinxiangCategory catL1 : catL1List) {
            List<JinxiangCategory> catL2List = categoryService.queryByPid(catL1.getId());
            List<Integer> l2List = new ArrayList<>();
            for (JinxiangCategory catL2 : catL2List) {
                l2List.add(catL2.getId());
            }

            List<JinxiangGoods> categoryGoods;
            if (l2List.size() == 0) {
