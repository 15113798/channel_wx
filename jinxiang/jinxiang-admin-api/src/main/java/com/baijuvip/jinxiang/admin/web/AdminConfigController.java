...
public class AdminConfigController {
    ...
    @RequiresPermissions("admin:config:signin:list")
    @RequiresPermissionsDesc(menu={"配置管理" , "签到配置"}, button="详情")
    @GetMapping("/signin")
    public Object listSignin() {
        Map<String, String> data = systemConfigService.listSignin();
        return ResponseUtil.ok(data);
    }

    @RequiresPermissions("admin:config:signin:updateConfigs")
    @RequiresPermissionsDesc(menu={"配置管理" , "签到配置"}, button="编辑")
    @PostMapping("/signin")
    public Object updateSignin(@RequestBody String body) {
        Map<String, String> data = JacksonUtil.toMap(body);
        systemConfigService.updateConfig(data);
        return ResponseUtil.ok();
    }
