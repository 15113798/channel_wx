package com.baijuvip.jinxiang.core.qcode;
...
public class QCodeService {
    ...
    public String createGrouponShareImage(String goodName, String goodPicUrl, JinxiangGroupon groupon) {
        try {
            //创建该商品的二维码
            ...
            //存储分享图
            JinxiangStorage storageInfo = storageService.store(inputStream2, imageData.length, "image/jpeg", getKeyName(groupon.getId().toString()));
    ...
    public String createGoodShareImage(String goodId, String goodPicUrl, String goodName) {
        ...
        try {
            ...
            ByteArrayInputStream inputStream2 = new ByteArrayInputStream(imageData);
            //存储分享图
            JinxiangStorage jinxiangStorage = storageService.store(inputStream2, imageData.length, "image/jpeg", getKeyName(goodId));

            return jinxiangStorage.getUrl();
        } catch (WxErrorException e) {
            e.printStackTrace();
        } catch (FileNotFoundException e) {
            e.printStackTrace();
