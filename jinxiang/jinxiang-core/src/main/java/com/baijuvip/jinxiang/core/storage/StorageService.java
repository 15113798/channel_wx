package com.baijuvip.jinxiang.core.storage;
...
public class StorageService {
     ...
     */
    public JinxiangStorage store(InputStream inputStream, long contentLength, String contentType, String fileName) {
        String key = generateKey(fileName);
        storage.store(inputStream, contentLength, contentType, key);

        String url = generateUrl(key);
        JinxiangStorage storageInfo = new JinxiangStorage();
        storageInfo.setName(fileName);
        storageInfo.setSize((int) contentLength);
        storageInfo.setType(contentType);
        storageInfo.setKey(key);
        storageInfo.setUrl(url);
        jinxiangStorageService.add(storageInfo);
