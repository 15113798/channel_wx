package com.baijuvip.jinxiang.core.system;

import com.baijuvip.jinxiang.core.util.SystemInfoPrinter;
import com.baijuvip.jinxiang.db.service.JinxiangSystemConfigService;
...
class SystemInistService {
    ...
    private void inist() {
        ...
        initConfigs();
        SystemInfoPrinter.printInfo("Jinxiang 初始化信息", getSystemInfo());
    }
    ...
    static {
        ...
        // 订单相关配置默认值
        DEFAULT_CONFIGS.put(SystemConfig.LITEMALL_MALL_NAME, "jinxiang");
    ...
    @Autowired
    private JinxiangSystemConfigService jinxiangSystemConfigService;

    private void initConfigs() {
        // 1. 读取数据库全部配置信息
        Map<String, String> configs = jinxiangSystemConfigService.queryAll();
        ...
        for (Map.Entry<String, String> entry : DEFAULT_CONFIGS.entrySet()) {
            ...
            jinxiangSystemConfigService.addConfig(entry.getKey(), entry.getValue());
        }
    ...
    private Map<String, String> getSystemInfo() {
        ...
        infos.put("调试级别", environment.getProperty("logging.level.com.baijuvip.jinxiang.wx"));

        // 测试获取application-core.yml配置信息
        infos.put(SystemInfoPrinter.CREATE_PART_COPPER + 1, "模块状态");
        infos.put("邮件", environment.getProperty("jinxiang.notify.mail.enable"));
        infos.put("短信", environment.getProperty("jinxiang.notify.sms.enable"));
        infos.put("模版消息", environment.getProperty("jinxiang.notify.wx.enable"));
        infos.put("快递信息", environment.getProperty("jinxiang.express.enable"));
        infos.put("快递鸟ID", environment.getProperty("jinxiang.express.appId"));
        infos.put("对象存储", environment.getProperty("jinxiang.storage.active"));
        infos.put("本地对象存储路径", environment.getProperty("jinxiang.storage.local.storagePath"));
        infos.put("本地对象访问地址", environment.getProperty("jinxiang.storage.local.address"));
        infos.put("本地对象访问端口", environment.getProperty("jinxiang.storage.local.port"));

        // 微信相关信息
        infos.put(SystemInfoPrinter.CREATE_PART_COPPER + 2, "微信相关");
        infos.put("微信APP KEY", environment.getProperty("jinxiang.wx.app-id"));
        infos.put("微信APP-SECRET", environment.getProperty("jinxiang.wx.app-secret"));
        infos.put("微信支付MCH-ID", environment.getProperty("jinxiang.wx.mch-id"));
        infos.put("微信支付MCH-KEY", environment.getProperty("jinxiang.wx.mch-key"));
        infos.put("微信支付通知地址", environment.getProperty("jinxiang.wx.notify-url"));
